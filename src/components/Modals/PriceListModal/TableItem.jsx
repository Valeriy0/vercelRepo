    ...
    >
      <div className="flex space-x-4">
        ...
        {activeLvlInfo && (
           <div className={`flex space-x-2`}>
            ...
            <span className="text-white-500"><Timer time={activeLvlInfo?.expiredAt} /> </span>
          </div>
        )}
      </div>
      {isReadyToActivate && (
        <button
          className={`bg-primary-500 w-[215px] py-2 flex items-center justify-center rounded-xl`}
        >
      ...
      <div className="flex items-center space-x-2.5 sm:py-2">
        ...
        <div className="flex items-center justify-center py-2 px-3 bg-white-700 shadow-item backdrop-blur-large rounded-xl">
          <span className="text-white opacity-50 text-xs font-medium">{PROGRAM_EXPIRED?.[PROGRAM_NAMES.MATRIX_B]?.[lvl]} days</span>
        </div>
